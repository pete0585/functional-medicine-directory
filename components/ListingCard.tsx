      ...
      <div className={`card-hover p-5 h-full flex flex-col ${isFeatured ? 'ring-2 ring-amber-400' : ''}`}>
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex-1 min-w-0">
            ...
            {listing.practice_name && listing.practice_name !== listing.full_name && (<p className="text-xs text-slate-500 truncate mt-0.5">{listing.practice_name}</p>)}
          </div>
          <div className="flex flex-col items-end gap-1 shrink-0">
            {listing.practitioner_type && (<span className="badge-teal text-xs">
        ...
        <div className="space-y-1.5 text-sm text-slate-500 flex-1">
          {(listing.city || listing.state) && (<div className="flex items-center gap-1.5">
              <MapPin className="h-3.5 w-3.5 text-teal-500 shrink-0"/>
              <span className="truncate">
                {[listing.city, listing.state].filter(Boolean).join(', ')}
              </span>
            </div>)}
          {listing.phone && (<div className="flex items-center gap-1.5">
              <Phone className="h-3.5 w-3.5 text-teal-500 shrink-0"/>
              <span>{formatPhone(listing.phone)}</span>
            </div>)}
          {listing.website && (<div className="flex items-center gap-1.5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {listing.is_ifm_certified && (<span className="badge-amber text-xs">IFM Certified</span>)}
          {listing.telehealth_available && (<span className="badge bg-teal-50 text-teal-700 text-xs">Telehealth</span>)}
          {listing.accepting_new_patients && (<span className="flex items-center gap-1 text-xs text-teal-600">
