...
export default function ListingDetail({ listing, monthlyViews }: ListingDetailProps) {
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-6">
        <div className={`card p-8 ${isFeatured ? 'ring-2 ring-amber-400' : ''}`}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              {listing.practice_name && listing.practice_name !== listing.full_name && (<p className="text-slate-500 mt-1">{listing.practice_name}</p>)}
              <div className="flex flex-wrap items-center gap-2 mt-3">
                {listing.practitioner_type && (<span className="badge-teal">
                    {PRACTITIONER_TYPES[listing.practitioner_type] ?? listing.practitioner_type.toUpperCase()}
                  </span>)}
                {null}
                {listing.is_ifm_certified && (<span className="badge-amber flex items-center gap-1">
          ...
          {listing.specialties && listing.specialties.length > 0 && (<div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {listing.specialties.map((s, i) => (<span key={i} className="badge-teal">{s}</span>))}
              </div>
            </div>)}

          {listing.conditions_treated && listing.conditions_treated.length > 0 && (<div className="mt-6">
              <h2 className="text-sm font-semibold text-slate-700 mb-3">Conditions Treated</h2>
              <div className="flex flex-wrap gap-2">
                {listing.conditions_treated.map((c, i) => (<span key={i} className="badge bg-cream-200 text-slate-700">{c}</span>))}
              </div>
            </div>)}
        </div>
        ...
        {!isClaimed && (<div className="card p-6 border-dashed border-2 border-cream-300 bg-teal-50">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <Link href={`/claim/${listing.id}`} className="btn-primary mt-3 text-sm py-2">
                  Claim This Listing
                </Link>
              </div>
            </div>
      ...
      <div className="space-y-4">
        <div className="card p-6">
          ...
          <div className="space-y-3">
            {(listing.city || listing.state) && (<div className="flex items-start gap-3">
                <MapPin className="h-4 w-4 text-teal-600 mt-0.5 shrink-0"/>
                <div>
                  {listing.address && (<p className="text-sm text-slate-600">{listing.address}</p>)}
            ...
            {(<>
                {listing.phone && (<div className="flex items-center gap-3">
                ...
                {listing.website && (<div className="flex items-center gap-3">
                    ...
                    <a href={listing.website.startsWith('http') ? listing.website : `https://${listing.website}`} target="_blank" rel="noopener noreferrer" className="text-sm text-teal-700 hover:text-teal-900 font-medium truncate">
        ...
        <div className="card p-6">
          ...
          <ul className="space-y-2.5">
            ...
            {listing.is_ifm_certified && (<li className="flex items-center gap-2.5 text-sm">
